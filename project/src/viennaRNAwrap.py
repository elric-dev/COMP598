...
def runRNAFold(seq):
    (tmpFileOut, tmpFileIn, struct, energy) = (
        "tmpOut.dat", "tmpIn.dat", None, None)
...
def mfeRNAFold(seq):
    (tmpFileOut, tmpFileIn, freq) = ("tmpOut.dat", "tmpIn.dat", None)
    inFile = open(tmpFileIn, "w")
    inFile.write(seq + "\n")
    inFile.close()
    os.system("%s -p > %s < %s" % (RNAFOLDwrapper, tmpFileOut, tmpFileIn))
    with open(tmpFileOut, "r") as fin:
        all_lines = fin.readlines()
    freq = float(all_lines[-1].split()[6][:-1])
    os.remove("rna.ps")
    os.remove(tmpFileOut)
    os.remove(tmpFileIn)
    return freq
